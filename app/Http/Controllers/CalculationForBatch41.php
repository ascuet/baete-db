...
class CalculationForBatch40 extends Controller
{
    public function getData(){    
        $results = DB::select("SELECT DISTINCT(roll)
            FROM student_infos
            where roll IN 
            (SELECT roll from student_infos WHERE course='DM' AND semester='Fall' AND year='2022')
            AND roll IN
            (SELECT roll from student_infos WHERE course='SP' AND semester='Fall' AND year='2022')
            AND roll IN
            (SELECT roll from student_infos WHERE course='SPL' AND semester='Fall' AND year='2022')
            AND roll IN
            (SELECT roll from student_infos WHERE course='ADA' AND semester='Fall' AND year='2023')
            AND roll IN
            (SELECT roll from student_infos WHERE course='ADAL' AND semester='Fall' AND year='2023')
            ");
            ...
            return view('data_41', compact('students_info', 'pos_total', 'count_po'));
        }
        ...
        private function calculatePO($roll){
            $pos_student = DB::select("SELECT 
                    ...
                    AND 
                    (
                        (course='DM' and semester='Fall' and year='2022')
                        OR (course='SP' and semester='Fall' and year='2022')
                        OR (course='SPL' and semester='Fall' and year='2022')
                        OR (course='ADA' and semester='Fall' and year='2023')
                        OR (course='ADAL' and semester='Fall' and year='2023')
                    )");
