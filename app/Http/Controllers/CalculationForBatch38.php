...
use DB;
class CalculationForBatch38 extends Controller
{
    public function getData(){    
        $results = DB::select("SELECT DISTINCT(roll)
            ...
            AND roll IN
            (SELECT roll from student_infos WHERE course='MML' AND semester='Fall' AND year='2022')
            AND roll IN
            (SELECT roll from student_infos WHERE course='SEISD' AND semester='Fall' AND year='2022')
            AND roll IN
            (SELECT roll from student_infos WHERE course='SEISDL' AND semester='Fall' AND year='2022')
            ");
            ...
            foreach($results as $r){
                ...
                // if($roll=="1903710201907" && $name=="Saqlain Absar Rafi"){
                //     echo $name;
                //     echo '<pre>';
                //     print_r($pos_total);
                // }
        ...
        private function calculatePO($roll){
            $pos_student = DB::select("SELECT 
                    ...
                    AND 
                    (
                           (course='ADA' and semester='Spring' and year='2022')
